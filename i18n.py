...
def resolve_language_preference(
    ...
    normalized_preference = preference.strip().casefold()

    if not normalized_preference or normalized_preference == SYSTEM_LANGUAGE:
        return language_code(system_locale)

    return language_code(normalized_preference)
...
def discover_translation_languages(
    ...
    if translations_dir.exists():
        pattern = f"{TRANSLATION_FILE_PREFIX}*.qm"

        for path in translations_dir.glob(pattern):
            stem = path.stem
            code = stem.removeprefix(TRANSLATION_FILE_PREFIX)

            if code:
                languages.add(language_code(code))
...
class TranslationManager:
    ...
    def available_languages(
        ...
        return discover_translation_languages(self._translations_dir)
    ...
    def apply_language(
        ...
        self.preference = preference.strip() or SYSTEM_LANGUAGE

        resolved_language = resolve_language_preference(
            self.preference,
            QLocale.system().name(),
        )
        ...
        if resolved_language == SOURCE_LANGUAGE:
            self.active_language = SOURCE_LANGUAGE
            return self.active_language

        translation_path = self._translations_dir / (
            f"{TRANSLATION_FILE_PREFIX}" f"{resolved_language}.qm"
        )

        if self._translator.load(str(translation_path)):
            self._app.installTranslator(self._translator)
            self._translator_installed = True
            self.active_language = resolved_language
        else:
