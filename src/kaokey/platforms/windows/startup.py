...
def startup_target() -> tuple[
    ...
    executable = _source_python_executable(Path(sys.executable))
...
def startup_folder() -> Path:
    ...
    return Path(appdata).joinpath(*STARTUP_FOLDER_PARTS)
...
def is_startup_enabled(
    ...
    if startup_shortcut_path(app_name).exists():
        return True

    return _legacy_registry_entry_exists(app_name)
...
def migrate_legacy_startup_entry(
    ...
    shortcut_path = startup_shortcut_path(app_name)
    ...
    if not _legacy_registry_entry_exists(app_name):
        return
...
def set_startup_enabled(
    ...
    if sys.platform != "win32":
        raise RuntimeError(
            "Windows startup integration " "is only available on Windows."
        )
...
def _create_startup_shortcut(
    ...
    executable, arguments, working_directory = startup_target()
    ...
    should_uninitialize = initialized in {
        S_OK,
        S_FALSE,
    }
    ...
    try:
        ...
        result = set_icon_location(
            shell_link,
            str(startup_icon_path(icon_path)),
            0,
        )
        ...
        result = query_interface(
            shell_link,
            ctypes.byref(IID_IPERSIST_FILE),
        ...
        if persist_file.value:
            _release_com_object(persist_file)

        if shell_link.value:
            _release_com_object(shell_link)

        if should_uninitialize:
            ole32.CoUninitialize()

    if not shortcut_path.exists():
        raise OSError("Windows startup shortcut was not created.")
...
def _com_method(
    ...
    *argtypes: Any,
    ...
    if not interface.value:
        raise OSError("COM interface is null.")
...
def _check_hresult(
    ...
    raise OSError(f"{message}: " f"HRESULT 0x{result & 0xFFFFFFFF:08X}")
