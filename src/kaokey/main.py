...
def main() -> None:
    start_hidden = STARTUP_LAUNCH_ARGUMENT in sys.argv[1:]
    ...
    settings = SettingsManager.create_default()

    translations = TranslationManager(app)
    translations.apply_language(settings.language)

    app.setStyle(KaokeyStyle(app.style()))
