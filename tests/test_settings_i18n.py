...
def test_settings_language_round_trip() -> None:
    ...
    assert store.data[SETTINGS_LANGUAGE_KEY] == "ru"
...
def test_show_hints_accepts_saved_string_false() -> None:
    store = MemorySettingsStore()
    store.data[SETTINGS_SHOW_HINTS_KEY] = "false"
...
def test_discover_translation_languages(
    ...
    (tmp_path / "kaokey_ru.qm").touch()
    (tmp_path / "kaokey_ja.qm").touch()
    (tmp_path / "something_else.qm").touch()

    assert discover_translation_languages(tmp_path) == [
        "en",
        "ja",
        "ru",
    ]
